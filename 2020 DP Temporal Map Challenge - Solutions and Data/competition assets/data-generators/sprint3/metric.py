...
PERMUTATIONS = [
    ALWAYS_GROUP_BY + [c1, c2] for c1 in MARGINAL_COLS for c2 in MARGINAL_COLS if c1 != c2
]
...
class TidyFormatKMarginalMetric:
    ...
    def scaled_k_marginal_score(self):
        ...
        if bias_mask.sum():
            logger.warning(f"warning: {bias_mask.sum()} place/times received a bias penalty")
    ...
    def get_bias_mask(self):
        ...
        allowable_errors = np.clip(
            counts.iloc[:, 1] * 0.05, a_min=K_MARGINAL_BIAS_PENALTY_MINIMUM, a_max=np.inf
        )
    ...
    def higher_order_conjunction(self, n_iters=HIGHER_ORDER_CONJUNCTION_ITERS):
        ...
        pivoted_dp = pivoted_dp.reindex(columns=pivoted_gt.columns).fillna(0).values.astype(np.int)
...
def score_submission(
    ...
    if parameters_json is not None:
        ...
        # logger.debug("checking that submission meets length limits and has proper epsilons ...")
